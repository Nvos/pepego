import {CreateTagComponent, EditTagComponent, TagsComponent, TagsTags} from "@libs/models";
...
const TagFieldEdit: React.FunctionComponent<TagFieldProps> = ({tag}: TagFieldProps) => {
...
const TagFieldCreate: React.FunctionComponent = () => {
  ...
  return <CreateTagComponent update={(cache, {data: {createTag}}) => {
  ...
  return <TagsComponent>
    {({data, loading, error}) => (
      <div>
        ...
        <React.Fragment>
            <p>Edit tags</p>
          {data.tags.map(it => <TagFieldEdit key={it.id} tag={it}/>)}
        </React.Fragment>
        }
        <p>Create tag</p>
        <TagFieldCreate/>
